...
export class RenderService {
  ...
  async renderBarChart(data: BarChartConfig): Promise<RenderResult> {
    ...
    try {
      ...
      const templatePath = path.join(this.viewsPath, 'components/bar_chart_with_title.ejs');
      const html = await ejs.renderFile(templatePath, data);
      const renderTime = Date.now() - startTime;
      logger.debug(`Bar chart rendered in ${renderTime}ms`, { chartId: data.chartId });
      ...
      logger.error('Error rendering bar chart', { error: (error as Error).message, data });
      throw new Error(`Failed to render bar chart: ${(error as Error).message}`);
    }
  }

  async renderTable(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    
    try {
      const templatePath = path.join(this.viewsPath, 'components/tabla-dificultad-analisis.ejs');
      ...
      const renderTime = Date.now() - startTime;
      logger.debug(`Table rendered in ${renderTime}ms`);
      ...
      logger.error('Error rendering table', { error: (error as Error).message, data });
      throw new Error(`Failed to render table: ${(error as Error).message}`);
    }
  }

  async renderComparative(data: ComparativeData & { chartId?: string }): Promise<RenderResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      logger.debug('Comparative chart data validation', { 
        hasLabels: defaultChartData.labels.length > 0,
        datasetCount: defaultChartData.datasets.length,
        chartId: data.chartId 
      });

      const templatePath = path.join(this.viewsPath, 'components/comparativo-puntaje.ejs');
      ...
      const renderTime = Date.now() - startTime;
      logger.debug(`Comparative chart rendered in ${renderTime}ms`, { chartId: data.chartId });
      ...
      logger.error('Error rendering comparative chart', { error: (error as Error).message, data });
      throw new Error(`Failed to render comparative chart: ${(error as Error).message}`);
    }
  }

  async renderLayout(data: LayoutData & { components?: ComponentRequest[] }): Promise<RenderResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const templatePath = path.join(this.viewsPath, layoutMap[layout]);
      ...
      logger.debug(`Layout rendered in ${renderTime}ms`, { 
        layout, 
        componentCount 
      });
      ...
      logger.error('Error rendering layout', { error: (error as Error).message, data });
      throw new Error(`Failed to render layout: ${(error as Error).message}`);
    }
  }

  async renderMultipleCharts(charts: BarChartConfig[]): Promise<RenderResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      logger.debug(`Multiple charts rendered in ${totalRenderTime}ms`, { 
        count: charts.length 
      });
      ...
      logger.error('Error rendering multiple charts', { error: (error as Error).message });
      throw new Error(`Failed to render multiple charts: ${(error as Error).message}`);
    }
  }

  async renderScoreDistribution(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    
    try {
      const templatePath = path.join(this.viewsPath, 'components/score_distribution.ejs');
      const html:any = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Score distribution rendered in ${renderTime}ms`);
      ...
      logger.error('Error rendering score distribution', { error: (error as Error).message, data });
      throw new Error(`Failed to render score distribution: ${(error as Error).message}`);
    }
  }

  async renderScoreDistributionHorizontal(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    
    try {
      const templatePath = path.join(this.viewsPath, 'components/score_distribution_horizontal.ejs');
      const html:any = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Score distribution horizontal rendered in ${renderTime}ms`);
      ...
      logger.error('Error rendering score distribution horizontal', { error: (error as Error).message, data });
      throw new Error(`Failed to render score distribution horizontal: ${(error as Error).message}`);
    }
  }

  async renderBarChartSimple(data: BarChartConfig): Promise<RenderResult> {
    const startTime = Date.now();

    
    try {
      const templatePath = path.join(this.viewsPath, 'components/bar_chart_simple.ejs');
      const html = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Bar chart simple rendered in ${renderTime}ms`, { chartId: data.chartId });
      ...
      logger.error('Error rendering bar chart simple', { error: (error as Error).message, data });
      throw new Error(`Failed to render bar chart simple: ${(error as Error).message}`);
    }
  }

  async renderCompetenciasChart(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    

    
    try {
      const templatePath = path.join(this.viewsPath, 'components/competencias_chart.ejs');
      const html:any = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Competencias chart rendered in ${renderTime}ms`, { chartId: data.chartId });
      ...
      logger.error('Error rendering competencias chart', { error: (error as Error).message, data });
      throw new Error(`Failed to render competencias chart: ${(error as Error).message}`);
    }
  }

  async renderPage(data: PageRequest): Promise<RenderResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (data.components && data.components.length === 1 && data.components[0].type === 'portada') {
        ...
        logger.info(`Portada page rendered directly`, { 
          renderTime,
      ...
      const templatePath = path.join(this.viewsPath, layoutTemplate);
  ...
  async renderPortada(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    
    try {
      const templatePath = path.join(this.viewsPath, 'components/portada.ejs');
      const html:any = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Portada rendered in ${renderTime}ms`);
      ...
      logger.error('Error rendering portada', { error: (error as Error).message, data });
      throw new Error(`Failed to render portada: ${(error as Error).message}`);
    }
  }

  async renderTablaPuntaje(data: any): Promise<RenderResult> {
    const startTime = Date.now();
    
    try {
      const templatePath = path.join(this.viewsPath, 'components/tabla_con_puntaje.ejs');
      const html:any = await ejs.renderFile(templatePath, data);
      
      const renderTime = Date.now() - startTime;
      logger.debug(`Tabla con puntaje rendered in ${renderTime}ms`, { 
      ...
      logger.error('Error rendering tabla con puntaje', { error: (error as Error).message, data });
      throw new Error(`Failed to render tabla con puntaje: ${(error as Error).message}`);
    }
  }
